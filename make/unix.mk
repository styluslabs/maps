...
# generate compile_commands.json needed by clangd; needs absolute path in directory
CLANGD=$(SRCBASE:%=$(OBJDIR)/%.clangd)

$(CLANGD): | $(BUILDDIRS)

$(OBJDIR)/%.clangd: %.cpp
	echo '  { "file": "$<", "directory": "$(PWD)", "command": "clang++ $(CFLAGS) $(CXXFLAGS) $(INCFLAGS) -o $(@:%.clangd=%.o) -c $<" },' > $@

$(OBJDIR)/%.clangd: %.cc
	echo '  { "file": "$<", "directory": "$(PWD)", "command": "clang++ $(CFLAGS) $(CXXFLAGS) $(INCFLAGS) -o $(@:%.clangd=%.o) -c $<" },' > $@

$(OBJDIR)/%.clangd: %.c
	echo '  { "file": "$<", "directory": "$(PWD)", "command": "clang $(CFLAGS) $(CCFLAGS) $(INCFLAGS) -o $(@:%.clangd=%.o) -c $<" },' > $@

compile_commands: $(CLANGD)
	(echo '['; cat $^; echo '  { "file": "__swallow_comma__", "directory": "$(PWD)", "command": "/usr/bin/true" } ]\n') > compile_commands.json
	rm $^
