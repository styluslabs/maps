...
class MapsView extends SurfaceView implements SurfaceHolder.Callback
{
  ...
  public boolean onTouchEvent(MotionEvent event)
  {
    ...
    case MotionEvent.ACTION_MOVE:
      ...
      //    sendTouchEvent(event, action, i, event.getHistoricalEventTime(j),
      //        event.getHistoricalX(i, j), event.getHistoricalY(i, j), event.getHistoricalPressure(i, j));
      //  }
      //}
      for (int i = 0; i < pointerCount; i++) {
    ...
    case MotionEvent.ACTION_DOWN:
      ...
      sendTouchEvent(event, action, 0, event.getEventTime(), event.getX(0), event.getY(0), event.getPressure(0));
      break;
