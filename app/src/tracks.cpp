...
void TrackSparkline::draw(SvgPainter* svgp) const
{
  ...
  int plotw = w - 10;
  int ploth = h - 15;
  p->clipRect(Rect::ltrb(6, 0, w, h-15));  // clip plot to axes
  p->translate(10, 0);
...
void MapsTracks::updateStats(std::vector<Waypoint>& locs)
{
  ...
  std::string timeStr = fstring("%dh %dm %ds", hours, mins, secs);
...
void MapsTracks::addWaypointItem(Waypoint& wp, const std::string& nextuid)
{
  ...
  Button* item = createListItem(MapsApp::uiIcon("waypoint"), wpname.c_str(), wp.desc.c_str());
  Widget* container = item->selectFirst(".child-container");
  //Button* showBtn = createToolbutton(MapsApp::uiIcon("eye"), "Show");
  ...
  item->onClicked = [=](){
    if(activeTrack == &navRoute) {
      ...
      bool wasTapToAdd = tapToAddWaypt;
      tapToAddWaypt = false;
...
Button* MapsTracks::createPanel()
{
  ...
  statsWayptsBtn->onClicked = [=](){ app->popPanel(); populateWaypoints(activeTrack); };
  ...
  wayptsOverflow->addItem("Rename", [=](){ editRouteContent->setVisible(true); });
